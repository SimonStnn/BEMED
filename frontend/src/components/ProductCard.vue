...
<template>
  <v-card border flat>
    ...
    <v-list-item v-else @click="$router.push({ name: 'product-details', query: { id: product.id } })" v-ripple
      :style="{ cursor: inDetailsPage ? 'default' : 'pointer', padding: 0 }">
      <v-card-title>
        {{ product.name }}
      </v-card-title>
      <v-card-subtitle>
        ...
        <v-menu v-for="alternative in product.alternatives" :key="alternative.id">
          <template v-slot:activator="{ props: menu }">
            <v-list-item @click.left="$router.push({ path: '/product', query: { id: alternative.id } })"
              @click.right.prevent="menu.onClick">
    ...
    <v-card-actions class="d-flex justify-end">
      ...
      <v-btn v-if="product.alternatives.length" color="primary" variant="flat"
        @click="()=>{
          ...
          $router.push({ name: 'assessmentsCreate', query: { productId: product.id} })}">Start Assessment</v-btn>
    </v-card-actions>
  </v-card>
</template>
